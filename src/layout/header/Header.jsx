...
export const Header = ({ favoritesCount = 0 }) => {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-zinc-100 bg-white/80 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1800px] items-center justify-between gap-3 px-4 py-3 sm:px-6 sm:py-4 lg:px-8">
        ...
        <nav className="flex items-center gap-2 sm:gap-3">
          ...
          <button className="flex h-[42px] w-[42px] cursor-pointer items-center justify-center rounded-2xl border border-zinc-200 bg-white text-zinc-600 shadow-sm transition-all hover:bg-zinc-50 hover:text-zinc-900 max-[840px]:h-[42px] max-[840px]:w-[42px] sm:h-auto sm:w-auto sm:gap-2 sm:px-4 sm:py-2.5">
            <User size={20} strokeWidth={2.5} />
            <span className="hidden text-base font-medium sm:block">Войти</span>
          </button>
        </nav>
      </div>
    </header>
